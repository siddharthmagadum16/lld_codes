...
class PricingService {
  ...
  private stratergyManagerService: StratergyManager = StratergyManager.getInstance();
  ...
  public getPriceEstimate(startLoc: Location, endLoc: Location): Record<VehicleType, number> {
    const idealCost = this.stratergyManagerService.getPricingStrategy().getPriceEstimate(startLoc, endLoc);
